 ...
 *
 * Promoted from services/superadmin/src/lib/supabase-admin.ts (which now
 * re-exports from here) so services/realtime's expiration cron can reuse
 * the exact same `setTenantUsersBanned` implementation instead of a second,
 * possibly-drifting copy — the same "revoke access" call the superadmin
 * service's own suspend/unblock/purge routes already rely on.
 *
 * SECURITY: this client bypasses Row Level Security and can list/ban/modify
 * any user across every tenant. It MUST only ever run inside a trusted
 * server-side process (services/superadmin behind requireSuperadmin(), or
 * services/realtime's cron job) — never expose SUPABASE_SECRET_KEY or this
